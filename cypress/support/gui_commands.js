...
describe("Cria Produto", () => {
  ...
  Cypress.Commands.add("cadastrarProdutoEstoque1", (produto) => {
    ...
    cy.get("input#code")
      .invoke("val")
      .then((valor) => {
        Cypress.env("codigoProduto", valor);
        cy.log(`Valor armazenado: ${valor}`);
      });
    cy.get("#name").type(produto.nome);
    cy.get("#value").type(produto.preco);
    cy.get(
      ":nth-child(2) > .input-group > .input-group-btn > .btn-primary"
    ).click();
    ...
    cy.get('input[name="product[1][amount]"]').clear().type("10,0");
    cy.get('input[name="product[1][cost]"]').clear().type("12,50");
    cy.get("button.btn-green").contains("Salvar").click();
  });
